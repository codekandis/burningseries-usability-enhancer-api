...
class UserSeriesWatchedAction extends AbstractWithDatabaseConnectorAction
{
	/**
	...
	public function execute(): void
	{
		...
		foreach ( $inputData[ 'seriesWatched' ] as $sentSeriesWatch )
		{
			/**
			 * @var SeriesEntity $seriesWatch
			 */
			$seriesWatch = SeriesEntity::fromObject( $sentSeriesWatch );

			$seriesDenial = $this->readSeriesDenialByNameAndUserId( $seriesWatch, $user );
			if ( null !== $seriesDenial )
			{
				$this->deleteSeriesDenialByIdAndUserId( $seriesDenial, $user );
			}

			$seriesInterest = $this->readSeriesInterestByNameAndUserId( $seriesWatch, $user );
			if ( null !== $seriesInterest )
			{
				$this->deleteSeriesInterestByIdAndUserId( $seriesInterest, $user );
			}

			$seriesFavorite = $this->readSeriesFavoriteByNameAndUserId( $seriesWatch, $user );
			if ( null !== $seriesFavorite )
			{
				$this->deleteSeriesFavoriteByIdAndUserId( $seriesFavorite, $user );
			}

			$this->writeSeriesWatchByNameAndUserId( $seriesWatch, $user );
		}
	 ...
	 * @throws PersistenceException
	 */
	private function writeSeriesWatchByNameAndUserId( SeriesEntity $seriesWatch, UserEntity $user ): void
	{
		( new SeriesWatchedRepository(
			$this->getDatabaseConnector()
		) )
			->writeSeriesWatchByNameAndUserId( $seriesWatch, $user );
	}
}
